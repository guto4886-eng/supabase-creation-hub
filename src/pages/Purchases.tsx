...
export default function Purchases() {
  ...
  const filtered = searched
    ? items.filter((item) => {
        ...
        if (filterStatus && item.payment_status !== filterStatus) return false;
        if (filterDateFrom && item.purchase_date < filterDateFrom) return false;
        if (filterDateTo && item.purchase_date > filterDateTo) return false;
        return true;
      })
    : [];
  ...
  const totalGeral = filtered.reduce((s, i) => s + (Number(i.total_price) || 0), 0);

  const saveMutation = useMutation({
    mutationFn: async (values: Record<string, any>) => {
      if (editing) {
        const { error } = await supabase.from("supplier_purchases").update(values).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("supplier_purchases").insert({ ...values, user_id: user!.id } as any);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["all_purchases"] });
      toast.success(editing ? "Compra atualizada com sucesso!" : "Compra registrada com sucesso!", { duration: 3000 });
      closeForm();
    },
  ...
  const handleClearFilters = () => {
    setFilterSupplier(""); setFilterDescription(""); setFilterObra(""); setFilterStatus(""); setFilterDateFrom(""); setFilterDateTo(""); setSearched(false); setPage(0);
  };
  ...
  const tableFields = [
    ...
    { name: "quantity_display", label: "Qtd" },
    { name: "unit_price_display", label: "Valor unit." },
    { name: "total_price_display", label: "Total" },
    { name: "purchase_date_display", label: "Data" },
    { name: "invoice_number", label: "NF" },
  ];
          ...
          <div className="flex flex-col h-full w-80">
            <div className="p-4 border-b border-border">
              <h2 className="text-lg font-bold text-primary uppercase flex items-center gap-2">
                <Search className="h-5 w-5" /> Compras
              </h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Status pagamento</label>
        ...
        {!searched ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="flex items-center gap-16 max-w-4xl px-8">
              ...
              <div className="text-center flex-1">
                ...
                <h3 className="text-xl font-semibold text-foreground mb-2">Registre uma nova compra!</h3>
                <p className="text-sm text-muted-foreground mb-4">Você também pode registrar uma nova compra agora.</p>
          ...
          <div className="flex-1 flex flex-col overflow-hidden p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                {filtered.length > 0 && (
                  <button onClick={() => exportToCSV(filtered.map(i => ({
                    ...i,
                    supplier_name: i.suppliers?.name || "",
                    obra_name: i.obras?.name || "",
                  })), tableFields, "compras")} className="flex items-center gap-2 px-3 py-2 border border-border text-foreground rounded-lg text-sm hover:bg-muted transition-colors">
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0">
                      <tr className="bg-muted/50">
                        ...
                        <th className="text-right px-3 py-3 font-medium text-muted-foreground">Valor unit.</th>
                        ...
                        <th className="text-left px-3 py-3 font-medium text-muted-foreground">NF</th>
                    ...
                    <tbody>
                      {paginatedItems.map((item, idx) => (
                        <tr key={item.id} onClick={() => openEdit(item)} className={`border-b border-border transition-colors cursor-pointer ${idx % 2 === 0 ? "bg-background" : "bg-muted/20"} hover:bg-muted/40`}>
                          <td className="px-3 py-2.5 text-foreground">{item.suppliers?.name || "—"}</td>
                          <td className="px-3 py-2.5 text-foreground truncate max-w-[200px]" title={item.description}>{item.description}</td>
                          ...
                          <td className="px-3 py-2.5 text-right text-muted-foreground">{formatCurrency(Number(item.unit_price) || 0)}</td>
                          <td className="px-3 py-2.5 text-right font-medium text-foreground">{formatCurrency(Number(item.total_price) || 0)}</td>
                          <td className="px-3 py-2.5 text-muted-foreground">{item.purchase_date ? new Date(item.purchase_date + "T00:00:00").toLocaleDateString("pt-BR") : "—"}</td>
                          <td className="px-3 py-2.5 text-muted-foreground">{item.invoice_number || "—"}</td>
                          ...
                          <td className="px-3 py-2.5" onClick={e => e.stopPropagation()}>
                            <div className="flex gap-1 justify-center">
                              ...
                              <button onClick={() => { if (confirm("Remover compra?")) deleteMutation.mutate(item.id); }} title="Excluir" className="p-1.5 rounded-md hover:bg-destructive/10 text-destructive"><Trash2 className="h-4 w-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={closeForm}>
          <div className="bg-card border border-border rounded-xl w-full max-w-3xl flex flex-col" style={{ maxHeight: "85vh" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted rounded-t-xl">
              <h3 className="text-lg font-semibold text-primary">{editing ? "Editar" : "Nova"} compra</h3>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Valor unitário</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Status pgto.</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Nº Nota Fiscal</label>
            ...
            <div className="flex justify-end gap-3 px-6 py-3 border-t border-border bg-muted rounded-b-xl">
              ...
              <button type="submit" form="" onClick={handleSubmit as any} disabled={saveMutation.isPending} className="px-5 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 disabled:opacity-50 flex items-center gap-2">
